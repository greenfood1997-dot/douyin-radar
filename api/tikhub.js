// Cloudflare Pages Functions format (converted from Vercel Edge)
// All original logic preserved - only export format changed
...
const CORS = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type,Authorization,x-api-key,anthropic-version,x-anthropic-key',
...
export async function onRequest(context) {
  ...
  // ── Anthropic 代理 (POST，无 endpoint) ──────────────────────────
  if (request.method === 'POST' && !searchParams.get('endpoint')) {
    const anthropicKey = request.headers.get('x-anthropic-key');
    if (!anthropicKey) {
      return new Response(JSON.stringify({ error: { message: 'Missing x-anthropic-key header' } }), { status: 401, headers: CORS });
    }
    try {
  ...
  try {
    ...
    if (endpoint === 'hot_search') {
      ...
      const tryFetch = async (url, label, body) => {
        try {
          const r = await fetchWithTimeout(url, { method: 'POST', headers: authHeaders, body: JSON.stringify(body) }, 15000);
          const j = await r.json();
          if (j?.code !== 200) return { label, code: j?.code, msg: j?.message_zh || j?.message, list: [] };
          const raw = j?.data?.aweme_list || j?.data?.data
      ...
      const debugLog = [r1, r2, r3, r4].map(r => ({ label: r.label, code: r.code, msg: r.msg, error: r.error, count: r.list.length }));
      const best = [r1, r2, r3, r4].find(r => r.list.length > 0);
      if (!best) {
        return ok({ type: 'search', keyword, title: `"${keyword}" 搜索结果`,
          updateTime: new Date().toLocaleString('zh-CN'), items: [], _debug: debugLog });
      }
      return ok({
        type: 'search', keyword, title: `"${keyword}" 搜索结果`,
        updateTime: new Date().toLocaleString('zh-CN'), _source: best.label,
      ...
      return ok({
        ...
        updateTime: new Date().toLocaleString('zh-CN'),
        user: {
          nickname: u.nickname || '', uniqueId: u.unique_id || uniqueId, uid: u.uid || '',
          signature: u.signature || '', avatarUrl: u.avatar_thumb?.url_list?.[0] || u.avatar_url || '',
          followerCount: u.follower_count || u.fans_count || 0, followingCount: u.following_count || 0,
          awemeCount: u.aweme_count || u.video_count || 0, totalFavorited: u.total_favorited || u.like_count || 0,
          verified: u.custom_verify || u.enterprise_verify_reason || '', region: u.region || u.country || '',
        },
      });
    }
